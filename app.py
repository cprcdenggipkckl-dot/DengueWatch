...
if uploaded_file is not None:
    with st.spinner('Sedang memproses data...'):
        try:
            ...
            # The exact HTML/JS code we built for true scaling
            html_template = """
            <!DOCTYPE html>
            <html>
            <head>
                ...
                <script>
                    ...
                    function updateMap() {
                        ...
                        if (visualMode === 'dots_400' || visualMode === 'dots_both') {
                            ...
                            layers.push({ sourcetype: 'geojson', source: geoJson400, type: 'fill', color: 'rgba(135, 206, 250, 0.15)' });
                            layers.push({ sourcetype: 'geojson', source: geoJson400, type: 'line', color: 'rgba(135, 206, 250, 0.8)', line: {width: 1} });
                        }

                        if (visualMode === 'dots_200' || visualMode === 'dots_both') {
                            const geoJson200 = createGeoJsonCircles(filteredData, 200);
                            layers.push({ sourcetype: 'geojson', source: geoJson200, type: 'fill', color: 'rgba(255, 69, 0, 0.2)' });
                            layers.push({ sourcetype: 'geojson', source: geoJson200, type: 'line', color: 'rgba(255, 69, 0, 0.8)', line: {width: 1} });
                        }
